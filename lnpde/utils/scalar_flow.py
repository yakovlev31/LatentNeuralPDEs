...
def create_argparser() -> argparse.ArgumentParser:
    ...
    # Data.
    parser.add_argument("--data_folder", type=str, default="./experiments/data/datasets/scalar_flow/partial/", help="Path to the dataset.")
    ...
    # Model (common).
    parser.add_argument("--d", type=int, default=5, help="Latent space dimension.")
    ...
    parser.add_argument("--boundary_cond", type=str, default="free", choices=["periodic", "free"], help="Boundary conditions for the spatial domain.")
    ...
    parser.add_argument("--F_hid_size", type=int, default=512, help="Hidden size for the dynamics function.")
    parser.add_argument("--F_n_hid_layers", type=int, default=3, help="Number of hidden layers for the dynamics function.")
    ...
    parser.add_argument("--delta_T", type=float, default=0.2, help="Attention time span for temporal aggregation function.")
    ...
    parser.add_argument("--delta_inf", type=float, default=0.5, help="Fraction of obsevations used for z0 inference at test time.")

    parser.add_argument("--model_folder", type=str, default="./models/scalar_flow/", help="Folder for saving/loading models.")
...
def get_model_components(param: SimpleNamespace) -> tuple[Encoder, ODETransitionFunction, NeuralDecoder]:
    ...
    g = NeuralDecoder(
        ...
        layers_to_count=[ExtractFirstComponent],
    )
...
def get_zk(elbo, t: list[Tensor], u: list[Tensor], Phi_h: Tensor, b_h: Tensor, k: int) -> Tensor:
    zk = []
    for i, (ti, ui) in enumerate(zip(t, u)):
        gamma, tau = elbo.q.h(ti, ti, ui, torch.stack([Phi_h[i]], dim=0), b_h[[i]])
        zk.append(gamma[:, [k], :, :] + tau[:, [k], :, :] * torch.randn_like(tau[:, [k], :, :]))
    return torch.cat(zk)
