...
import lnpde.utils.navier_stokes as data_utils
...
for i in tqdm(range(param.n_iters), total=param.n_iters):
    ...
    if i % int(0.01 * param.n_iters) == 0 or i == param.n_iters - 1:
        with torch.no_grad():
            ...
            L1_val, L2_val, L3_val, z_val, s_val = elbo(t_val, u_val, param.block_size, param.scaler, Phi_h_val, b_h_val, Phi_F_val, b_F_val)
            ...
            if param.visualize == 1:
                ...
                s_plot = torch.zeros_like(z[:, :-1])
                s_plot[:, ::param.block_size] = s
