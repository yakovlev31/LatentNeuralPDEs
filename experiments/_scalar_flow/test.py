...
with torch.no_grad():
    ...
    for batch in tqdm(test_loader, total=len(test_loader)):
        ...
        assert param.batch_size == 1, ""
        m = t_inf[0].shape[1]
        k = 8  # for k=9 we get zm
        u_pd = torch.zeros_like(u)[:, k:]
        _t = t[:, k:].clone()
        _t = _t - _t.min()
        for i in range(param.n_mc_samples):
            zk = data_utils.get_zk(elbo, t_inf, u_inf, Phi_h, b_h, k)
            u_pd += data_utils._pred_full_traj(elbo, _t, zk, Phi_F, b_F)
        u_pd /= param.n_mc_samples
        loss_per_traj = reduce(loss_fn(u_pd[:, m-k:], u[:, m:]), "s m n d -> s () () ()", "mean").view(-1).detach().cpu().numpy().ravel()
        losses.extend(loss_per_traj)
        ...
        data_utils.visualize_trajectories(
            ...
            traj=[
                ...
                u_pd[0, m-k:].detach().cpu().numpy(),
            ],
            vis_inds=list(range(0, u_pd[:, m-k:].shape[1]))[::],
